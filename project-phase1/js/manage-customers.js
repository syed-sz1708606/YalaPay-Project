...
async function addCustomer(event) {
    ...
    if (customerId) {
        ...
        window.alert(`Customer with ID: ${customer.customerId} updated.`);
    } else {
        const customerList = await customerRepo.getAllCustomers();
        customer.customerId = customerList[customerList.length - 1].customerId + 1;
        await customerRepo.addCustomer(customer);
        window.alert(`Customer with ID: ${customer.customerId} added.`);
    }
