...
async function searchPayment() {
    ...
    if (searchCriteria == "byId") {
        const paymentId = parseInt(searchInput);
        if (!paymentId) return window.alert(`Invalid search query.`);
        payment = await paymentRepo.getPaymentById(paymentId, invoiceId);
...
async function deletePayment(id) {
    ...
    if (data.paymentMode === "Cheque") await chequeRepo.deleteCheque(data.chequeNo)
    await displayPayment()
}
...
function sendChequeDetailsToCheque(cheque) {
    ...
    payChequeIssueDate.value = cheque.receivedDate
    payChequeDueDate.value = cheque.dueDate
...
async function addPayment() {
    ...
    let lastIndexPaymentId
    let cheque

    //adding new payment
    if (isNaN(paymentNo)) {
        const paymentData = await paymentRepo.getAllPayments()
        if (paymentData.length > 0) {
            lastIndexPaymentId = (paymentData[paymentData.length - 1].paymentId) + 1
            paymentNo = lastIndexPaymentId
        } else {
            lastIndexPaymentId = 1
            paymentNo = lastIndexPaymentId
        }
        payment = new Payment(paymentNo, invoiceId, paymentAmount.value, paymentDate.value, paymentMode.value, "none")
        payment.chequeNo = "none"
        ...
        if (paymentMode.value == "Cheque") {
            ...
            cheque = new Cheque(parseInt(payChequeId.value), parseInt(paymentAmount.value), payChequeDrawer.value, payBankSelect.value,
                payChequeStatus.value, payChequeIssueDate.value, payChequeDueDate.value, payImage.getAttribute("src"))

            payment.amount = parseInt(cheque.amount)

            //Duplicate ChequeNo validation
            const existingCheque = await chequeRepo.getChequeById(cheque.chequeNo)
            if (existingCheque) {
            ...
            if (!chequeErrorValidation(cheque)) {
                return
            }
        ...
        if (!paymentErrorValidation(payment)) {
            return
        }

        paymentId.value = parseInt(paymentNo)
        ...
        payment = new Payment(paymentNo, invoiceId, parseInt(paymentAmount.value), paymentDate.value, paymentMode.value, "none")
        ...
        if (isCheque == true) {     // updating cheque
            ...
            cheque.amount = parseInt(payment.amount)
...
async function editPayment(id) {
    ...
    paymentAmount.value = parseInt(payment.amount)
    paymentDate.value = payment.paymentDate
    paymentMode.disabled = true;
    editPaymentAmount = parseInt(payment.amount)

    if (paymentMode.value === "Cheque") { // if payment selected is a cheque

        await onChequeSelect()
        payChequeId.value = parseInt(payment.chequeNo)
        cheque = await chequeRepo.getChequeById(payment.chequeNo)
...
function chequeErrorValidation(cheque) {
    let valid = true;
...
function paymentErrorValidation(payment) {
    let valid = true;
...
async function loadBankOptions() {
    ...
    const getAllBankOptions = await response.json()
...
async function loadPaymentOptions() {
    const url = "../data/payment-modes.json"
    const response = await fetch(url)
    const getAllPaymentModes = await response.json()

    const getPayMode = getAllPaymentModes.map(payMode => `<option value="${payMode}">${payMode}</option>`)
